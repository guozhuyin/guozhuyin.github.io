...
function render(){
 ...
 const hints={publications:'題名、姓名與會議名稱只存一份原文，切換網站語言不會改動；摘要可另外翻譯。正式連結會直接套在論文原文題名上。',certificates:'正式名稱保留原文。證照名稱本身是展開證明圖片的入口；中文頁優先中文版，英文與其他語言優先英文版，缺少時回退到另一版本。',awards:'競賽名稱本身是展開證明圖片的入口。若有對應作品，填入作品代稱（例如 ai-cup-2022），成果區會提供作品頁連結。',articles:'全文保留在 WordPress；這裡保存每篇文章的標題、摘要、分類、日期與原文網址。勾選「放在首頁精選」後，首頁與文章頁最多顯示前三篇；未精選的文章仍保留在此清單，供日後挑選。',resources:'舊網址會保留在資料中，但公開網站只提供 HTTPS 連結；沒有可用連結時，作品名稱會以純文字顯示。',languages:'新增語言後，可先不啟用，在其他分類填寫翻譯。啟用後才會產生頁面；第一個啟用的語言是首頁預設。不要變更已有語言代稱，以免改變網址。',interface:'這裡管理導覽、按鈕與說明文字。未填的語言會使用替代語言；不會自動呼叫機器翻譯。',education:'求職頁面會先呈現大學與研究所；勾選「收合到早期教育（K–12）」的項目會放在可展開區塊。若有學歷證明，可填入證明名稱及中英文圖片，項目名稱會成為展開入口。學校／系所網站會直接套在右側名稱上，避免重複列出連結。'};$('#description').textContent=hints[section]||'一般內容可分別填寫各語言。空白翻譯保留另一語言的原文；不會改寫原始資料。';
